...
_OUTPUT_DIR = Path("Outputs") / "S-4_Subplots"
...
# Concentration mappings for each compound (only L concentrations for plotting)
CONCENTRATION_MAP = {
    'ACE': [73],  # Only L
    'ISO': [78],  # Only L
    'TOL': [83],  # Only L
...
CONC_LABELS = ["L"]  # Only L
CONC_COLORS = ['#008080', '#B7410E', '#C8A2D6']
...
def plot_ace_subplots(data_folder: Path):
    ...
    if not folder.exists():
        raise FileNotFoundError(f"Could not find folder for compounds at {folder}")
    
    fig, ax = plt.subplots(figsize=(6, 5))  # Only one axis for S-4
    
    export_data = pd.DataFrame(columns=["Time [s]", "Voltage", "Concentration", "Compound"])
    
    # Iterate through each compound (ACE, ISO, TOL)
    for compound, color in zip(COMPOUNDS, CONC_COLORS):
        concentrations = CONCENTRATION_MAP[compound]  # Get the concentrations for this compound
        
        for conc_idx, conc in enumerate(concentrations):  # Only plot L concentrations
            conc_label = CONC_LABELS[conc_idx]
            
            # Construct the path for this compound and concentration
            compound_folder = folder / f"{compound}_{conc}ppm"
            
            if not compound_folder.exists():
                logger.warning(f"Folder for {compound} {conc}ppm does not exist. Skipping.")
                continue
            
            # If folder exists, check each CSV file
            for csv_file in compound_folder.iterdir():
                if csv_file.is_file() and "csv" in csv_file.name:
                    ...
                    if SQ_TR_COL in tmp_data[TEMPERATURE_MODULATION_COL].unique():
                        ...
                        ax.plot(time_values, meas_volt_data_period, label=f"{compound} {conc_label}", color=color)
    ...
    # Export data separately for each compound
    for compound in COMPOUNDS:
        compound_data = export_data[export_data["Compound"] == compound]
        if not compound_data.empty:
            export_file_path = _DATA_EXPORT_DIR / f"S-4_{compound}.csv"
            compound_data[["Time [s]", "Voltage"]].copy().to_csv(export_file_path, index=False, header=False)
            logger.info(f"Data for {compound} exported to {export_file_path}")
