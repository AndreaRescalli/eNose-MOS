...
output_file_path = FEATURES_FOLDER / "MIX_features_norm.csv"
features_var_dropna_norm.to_csv(output_file_path)


######################
#         ML         #
######################
# Correlation matrix for initial features
...
# Stratified K-Fold
skf = StratifiedKFold(n_splits=3)  # Low n_splits due to small dataset
...
#with open(r"C:\Users\resca\OneDrive - Politecnico di Milano\_Dottorato\4 - Code\temperature-enose-data-analysis\mos-v2\Outputs\Features\TEST_R_NORM_ZSCORE\single_lda.pkl", "rb") as file:
    #lda = pk.load(file)
# Transform data using the pre-trained LDA
...
# Verify LDA-transformed feature correlations
...
model = CatBoostClassifier(
    ...
    random_seed=6,
    #train_dir=tensorboard_log_dir,  # Enable TensorBoard logging
...
importance = classifier.feature_importances_
sorted_indices = np.argsort(importance)[::-1]
sorted_lda_feature_names = [lda_feature_names[i] for i in sorted_indices]  # Sort LDA features by importance
plt.bar(range(len(importance)), importance[sorted_indices])
plt.xticks(range(len(importance)), sorted_lda_feature_names, rotation=90)
plt.xlabel("LDA Components")
plt.ylabel("Feature Importance")
...
metrics = ["Accuracy", "F1-Score", "Precision", "Recall"]
train_values = [train_accuracy, train_f1, train_precision, train_recall]
test_values = [accuracy, f1, precision, recall]

x = np.arange(len(metrics))  # Label locations
width = 0.35  # Bar width

plt.figure(figsize=(8, 6))
plt.bar(x - width/2, train_values, width, label="Training", color="blue")
